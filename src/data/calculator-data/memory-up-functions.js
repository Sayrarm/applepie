...
export const getUpgradeResources = (
  rarity,
  allLevels,
  startIndex,
  endIndex,
  ...
  const rarityData = rarityLevels[rarity];
  if (!rarityData)
    return { crystals: { N: 0, R: 0, SR: 0 }, credits: 0, heart: null };
...
export const getCrystalDungeonRuns = (crystalsNeeded, dungeonLevel) => {
  ...
  if (crystalsNeeded.N > 0 && dungeonCrystals.N > 0) {
    runsNeeded = Math.max(
      runsNeeded,
      Math.ceil(crystalsNeeded.N / dungeonCrystals.N),
    );
  }
  if (crystalsNeeded.R > 0 && dungeonCrystals.R > 0) {
    runsNeeded = Math.max(
      runsNeeded,
      Math.ceil(crystalsNeeded.R / dungeonCrystals.R),
    );
  }
  if (crystalsNeeded.SR > 0 && dungeonCrystals.SR > 0) {
    runsNeeded = Math.max(
      runsNeeded,
      Math.ceil(crystalsNeeded.SR / dungeonCrystals.SR),
    );
  }

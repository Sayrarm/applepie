...
export const calculateProtocoreStats = (protocores, baseStats) => {
    ...
    protocores.forEach(protocore => {
        ...
        switch (statName) {
            ...
            case 'DMG Boost to Weakened': bonuses.dmgBoostPercent += statValue; break;
            case 'Oath Strength': bonuses.oathStrengthPercent += statValue; break;
            case 'Oath Recovery Boost': bonuses.oathRecoveryBoostPercent += statValue; break;
            case 'Expedited Energy Boost': bonuses.expeditedEnergyBoostPercent += statValue; break;
            default: break;
        }
        ...
        if (protocore.substats) {
            protocore.substats.forEach(sub => {
                switch (sub.stat) {
                    ...
                    case 'DMG Boost to Weakened': bonuses.dmgBoostPercent += sub.value || 0; break;
                    case 'Oath Strength': bonuses.oathStrengthPercent += sub.value || 0; break;
                    default: break;
                }
            });
        }
...
export const calculateFinalStats = (card, baseStats, protocores) => {
    ...
    return {
        ...
        oathStrength: (baseStats.oathStrength || 0) + (protocoreStats.oathStrength || 0),
        oathRecoveryBoost: (baseStats.oathRecoveryBoost || 0) + (protocoreStats.oathRecoveryBoost || 0),
        expeditedEnergyBoost: (baseStats.expeditedEnergyBoost || 0) + (protocoreStats.expeditedEnergyBoost || 0)
    };
};

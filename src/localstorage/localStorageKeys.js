export const KEYS = {
  ...
  SHOW_USER_INFO: "show_user_info", //показывать информацию о карточке (уровень, ранг, возвышение)
...
export const getProtocoreFilterKeys = (prefix) => ({
  ...
  stellactrum: prefix
    ? `${prefix}_protocore_filter_stellactrum`
    : "protocore_filter_stellactrum",
  levels: prefix
    ? `${prefix}_protocore_filter_levels`
    : "protocore_filter_levels",
  mainStats: prefix
    ? `${prefix}_protocore_filter_mainStats`
    : "protocore_filter_mainStats",
  subStats: prefix
    ? `${prefix}_protocore_filter_subStats`
    : "protocore_filter_subStats",
  status: prefix
    ? `${prefix}_protocore_filter_status`
    : "protocore_filter_status",
  filters: prefix ? `${prefix}_protocore_filters` : "protocore_filters",
  search: prefix
    ? `${prefix}_protocore_search_query`
    : "protocore_search_query",
  sort: prefix
    ? `${prefix}_protocore_sort_criteria`
    : "protocore_sort_criteria",
});

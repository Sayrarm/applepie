  ...
  const parseText = (content) => {
    ...
    while ((match = regex.exec(content)) !== null) {
      ...
      parts.push({
        ...
        description:
          termData?.description || `There is no description for "${termKey}"`,
      });
      ...
      if (isMobile()) {
        ...
        if (isAbove) {
          ...
          const termRect =
            ...
            document
              .querySelector(
                `.${styles.glossaryTerm}[data-term="${activeTermRef.current}"]`,
              )
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (isMobile() && tooltip.visible) {
        ...
        const isTermClick =
          event.target.classList?.contains("glossaryTerm") ||
        ...
        {parts.map((part, index) => {
          ...
          return (
            <span
              ...
              // Десктопные события
              onMouseEnter={(e) =>
                handleMouseEnter(e, part.title, part.description)
              }
              onMouseLeave={handleMouseLeave}
              // Мобильные события
              onClick={(e) =>
                handleClick(e, part.title, part.description, part.key)
              }
      ...
      {tooltip.visible &&
        createPortal(
          ...
          >
            {/* Заголовок */}
            {tooltip.title && (
              <div className={styles.glossaryTooltipTitle}>{tooltip.title}</div>
            )}
            {/* Описание */}
            <div className={styles.glossaryTooltipText}>{tooltip.text}</div>
            {mobile && (
              <button
                onClick={() => {
                  setTooltip((prev) => ({ ...prev, visible: false }));

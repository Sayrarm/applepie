...
function CardProtocores({ cardId }) {
  ...
  const saveProtocores = useCallback(
    (protocores) => {
      if (!cardId) return;
  ...
  const canAddProtocore = useCallback(
    (protocore) => {
      ...
      if (
        (protocore.type === "alpha" || protocore.type === "beta") &&
        isLunar
      ) {
        alert(
          `Alpha and Beta protocores can only be equipped on SOLAR cards. This card is ${cardPlacement.toUpperCase()}.`,
        );
        return false;
      }
      ...
      if (
        (protocore.type === "gamma" || protocore.type === "delta") &&
        isSolar
      ) {
        alert(
          `Gamma and Delta protocores can only be equipped on LUNAR cards. This card is ${cardPlacement.toUpperCase()}.`,
        );
        return false;
      }
      ...
      if (cardStella && protocore.stellactrum !== cardStella) {
        alert(
          `This protocore has ${protocore.stellactrum} stellactrum, but the card requires ${cardStella}.`,
        );
        return false;
      }
  ...
  // Добавить протокор
  const handleAddProtocore = useCallback(
    (protocoreId) => {
  ...
  // Удалить протокор
  const handleRemoveProtocore = useCallback(
    (protocoreId) => {
      const updatedProtocores = selectedProtocores.filter(
        (p) => p.id !== protocoreId,
      );
      setSelectedProtocores(updatedProtocores);
      saveProtocores(updatedProtocores);
  ...
  const availableProtocores = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      return sortProtocores(
        filtered.filter((protocore) => {
          return (
            protocore.type.toLowerCase().includes(searchLower) ||
  ...
  const getLimitMessage = () => {
    ...
    const allowedTypes =
      cardPlacement === "solar" ? "Alpha, Beta" : "Gamma, Delta";
    const stellaText = cardStella
      ? ` (${cardStella.charAt(0).toUpperCase() + cardStella.slice(1)})`
      : "";
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        <div>
          ...
          <div className={styles.placementInfo}>{getLimitMessage()}</div>
        </div>
        ...
        <div className={styles.headerActions}>
          ...
          <button
            ...
            disabled={
              allProtocores.length === 0 || selectedProtocores.length >= 2
            }
          >
            + Add Protocore
          </button>

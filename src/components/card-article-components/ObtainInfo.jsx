...
import { ModalWindow, BannerList } from "@components";
...
function ObtainInfo({ cardId, obtainData }) {
  ...
  const banners = bannersDataFull.filter((b) =>
    b.cardIds.includes(Number(cardId)),
  );
        ...
        tag={
          <div className={styles.modalInfo}>
            <div className={styles.modalInfo}>
              ...
              <img
                src={getImageUrl(selectedObtain?.image)}
                alt={selectedObtain?.obtain}
                width={"50%"}
                height={"50%"}
              />
            </div>
            ...
            {selectedObtain &&
              BANNER_OBTAIN_TYPES.includes(selectedObtain.obtain) &&
              banners.length > 0 && (
                <>

...
const ModalWindowProtocore = forwardRef((props, ref) => {
  ...
  const handleOk = async () => {
    try {
      ...
      const substatsWithValues = values.substats
        .filter(
          (item) =>
            item.stat !== null && item.stat !== undefined && item.stat !== "",
        )
        .map((item) => {
      ...
      const protocoreData = {
        ...
        createdAt: editingProtocore
          ? editingProtocore.createdAt
  ...
  const availableMainStats = selectedType
    ? getAvailableMainStatsForType(selectedType)
    : [];
  ...
  return (
    <Modal
      ...
      title={
        editingProtocore
          ? `Edit ${editingProtocore.type.charAt(0).toUpperCase() + editingProtocore.type.slice(1)} Protocore`
          : title
      }
      onOk={handleOk}
    ...
    >
      ...
      >
        ...
        <Form.Item
          ...
          rules={[
            { required: true, message: "Please select stellactrum color!" },
          ]}
        >
          <Select
        ...
        <Form.Item
          ...
          rules={[
            { required: true, message: "Please select level!" },
            {
              ...
              min: 0,
              max: 15,
              message: "Level must be between 0 and 15!",
            },
          ]}
        >
          <InputNumber
        ...
        <div style={{ display: "flex", gap: "16px" }}>
                ...
                validator: (_, value) => {
                  ...
                  if (!type) {
                    return Promise.reject(
                      new Error("Please select protocore type first!"),
                    );
                  }
                  const availableStats = getAvailableMainStatsForType(type);
                  if (!availableStats.includes(value)) {
                    return Promise.reject(
                      new Error(
                        `This stat is not available for ${protocoreTypes[type]?.name || type}`,
                      ),
                    );
                  }
          ...
          <Form.Item
            ...
            rules={[
              { required: true, message: "Please enter main stat value!" },
            ]}
            style={{ flex: 1 }}
          >
            <InputNumber

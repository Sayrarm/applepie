...
function FarmGoalTracker() {
  ...
  const calculateCrystalsFarming = (neededCrystals, dungeonLevel) => {
    ...
    if (neededCrystals.N > 0 && dungeon.crystals.N > 0) {
      ...
      details.N = {
        needed: neededCrystals.N,
        runs: runsN,
        perRun: dungeon.crystals.N,
      };
    ...
    if (neededCrystals.R > 0 && dungeon.crystals.R > 0) {
      ...
      details.R = {
        needed: neededCrystals.R,
        runs: runsR,
        perRun: dungeon.crystals.R,
      };
    ...
    if (neededCrystals.SR > 0 && dungeon.crystals.SR > 0) {
      ...
      details.SR = {
        needed: neededCrystals.SR,
        runs: runsSR,
        perRun: dungeon.crystals.SR,
      };
  ...
  const calculateRemaining = (goal) => {
    ...
    const remainingCredits = Math.max(
      0,
      goal.neededCredits - (userResources.credits || 0),
    );
    ...
    if (
      goal.type === "memory" &&
      (goal.neededCrystalsN > 0 ||
        goal.neededCrystalsR > 0 ||
        goal.neededCrystalsSR > 0)
    ) {
    ...
    <div className={styles.container}>
      <div className={styles.goalsList}>
        {goals.map((goal) => {
          ...
          let expFarming;
          if (goal.type === "memory") {
            expFarming = calculateExpFarmingMemory(
              remaining.exp,
              expDungeonLvl,
            );
          } else {
            expFarming = calculateExpFarmingProtocore(
              remaining.exp,
              expDungeonLvl,
            );
          }

          const creditsFarming = calculateCreditsFarming(
            remaining.credits,
            creditDungeonLvl,
            goal.type === "protocore",
          );
          const crystalsFarming = remaining.crystals
            ? calculateCrystalsFarming(remaining.crystals, crystalDungeonLvl)
            : null;
              ...
              <div className={styles.goalContent}>
                ...
                <div className={styles.resourcesSection}>
                  ...
                  <div className={styles.resourcesList}>
                    {goal.neededExp > 0 && (
                      <div className={styles.expRow}>
                        {getExpLabel(goal, true)}{" "}
                        {goal.neededExp.toLocaleString()}
                      </div>
                    )}
                    {goal.type === "memory" &&
                      (goal.neededCrystalsN > 0 ||
                        ...
                        <div className={styles.crystalRow}>
                          <img
                            src={getImageUrl(
                              getCrystalColorIcon(goal.crystalColor),
                            )}
                          ...
                          {goal.neededCrystalsN > 0 &&
                            ` N: ${goal.neededCrystalsN}`}
                          {goal.neededCrystalsR > 0 &&
                            ` | R: ${goal.neededCrystalsR}`}
                          {goal.neededCrystalsSR > 0 &&
                            ` | SR: ${goal.neededCrystalsSR}`}
                        </div>
                      )}
                    {goal.type === "memory" &&
                      goal.heart &&
                      ...
                      <div className={styles.farmingDungeons}>
                        {remaining.exp > 0 && (
                          <div className={styles.farmingCard}>
                            <div className={styles.farmingItem}>
                              {getExpLabel(goal, false)}
                            </div>
                            <img
                            ...
                            />
                            <span className={styles.farmingItem}>
                              {expFarming.runs} runs (Lvl {expDungeonLvl})
                            </span>
                            <span className={styles.farmingValue}>
                              {" "}
                              {expFarming.stamina} stamina
                            </span>
                          </div>
                        )}
                        ...
                        {remaining.crystals &&
                          ...
                          (remaining.crystals.N > 0 ||
                            ...
                            <div className={styles.farmingCard}>
                              ...
                              <img
                                src={getImageUrl(
                                  getDungeonIcon(goal, goal.crystalColor),
                                )}
                                alt="crystals dungeon"
                                className={styles.itemIcon}
                              />
                              <span className={styles.farmingItem}>
                                {crystalsFarming.runs} runs (Lvl{" "}
                                {crystalDungeonLvl})
                              </span>
                              <span className={styles.farmingValue}>
                                {crystalsFarming.stamina} stamina
                              </span>
                            </div>
                          )}
                        ...
                        {remaining.credits > 0 && (
                          <div className={styles.farmingCard}>
                            ...
                            />
                            <span className={styles.farmingItem}>
                              {creditsFarming.runs} runs (Lvl {creditDungeonLvl}
                              )
                            </span>
                            <span className={styles.farmingValue}>
                              {creditsFarming.stamina} stamina
                            </span>
                          </div>
                        )}
                      </div>
                ...
                {remaining.exp === 0 &&
                  ...
                  (!remaining.crystals ||
                    (remaining.crystals.N === 0 &&
                      remaining.crystals.R === 0 &&
                      remaining.crystals.SR === 0)) && (
                    <div className={styles.enoughResources}>

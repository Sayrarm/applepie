...
export function useResources() {
    ...
    const [selectedCrystalColor, setSelectedCrystalColor] = useState(getSelectedCrystalColor);
    ...
    useEffect(() => { saveHeartsand(heartsand); }, [heartsand]);
    useEffect(() => { saveCrystals(crystals); }, [crystals]);
    useEffect(() => { saveCrystalBoxes(crystalBoxes); }, [crystalBoxes]);
    useEffect(() => { saveHearts(hearts); }, [hearts]);
    useEffect(() => { saveCoreEnergy(coreEnergy); }, [coreEnergy]);
    useEffect(() => { saveCredits(credits); }, [credits]);
    useEffect(() => { saveSelectedCrystalColor(selectedCrystalColor); }, [selectedCrystalColor]);
    useEffect(() => { saveDiamonds(diamonds); }, [diamonds]);
    useEffect(() => { saveWish(wish); }, [wish]);
